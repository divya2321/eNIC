	...
	function GetTemplateData() {
		...
		jQuery.ajax({
			...
			success : function(msg) {
				...
				if(msg.retValue == 0) {	
				 $('#fingerprintTemp').val(msg.templateBase64);
...
function showPosition(position) {
	...
	var pNic = $("#personNic").val();
	
	if(pNic){
		
  var u = "/org/location/track/save?nic="+pNic+"&lat="+position.coords.latitude+"&lon="+position.coords.longitude;
  ...
  $.ajax({
	...
	if(status=='success'){
			     $(location).attr('href', '/org/location/track/');
